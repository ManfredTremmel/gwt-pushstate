...
final class CodeServerParameterHelper {
  ...
  public static String append(final String token) {
    final StringBuilder result = new StringBuilder(token);

    /*
     * This gets compiled out in production mode!
     */
    if (!GWT.isProdMode() && GWT.isClient()) {
      final String gwtCodesvr = Window.Location.getParameter("gwt.codesvr");
      if (gwtCodesvr != null) {
        if (token.contains("?")) {
          result.append('&');
        } else {
        ...
        result.append("gwt.codesvr=").append(gwtCodesvr);
      }
    }
